...
make_lP_effect_at_aCO2_eCO2_chart <- function(sumDF2) {
    ...
    ### subset
    plotDF1 <- subset(sumDF2, Category == "Biomass")
    plotDF2 <- subset(sumDF2, Category == "Concentration")
    plotDF3 <- subset(sumDF2, Category == "Gas exchange")
    plotDF4 <- subset(sumDF2, Category == "Morphology")
    plotDF5 <- subset(sumDF2, Category == "Nutrient uptake")
    plotDF6 <- subset(sumDF2, Category == "Resource use efficiency")
    ...
    p1 <- ggplot(plotDF1)+ 
        ...
        scale_x_continuous(limits=c(-2.0, 0.4))+
    ...
    pdf("output/metafor_summary_plot/P_effect_at_aCO2_eCO2_all_results.pdf", width=6, height=20)
