make_nutrient_uptake_plots <- function(inDF) {
    
    if(!dir.exists("output/overall_nutrient_uptake")) {
        dir.create("output/overall_nutrient_uptake", showWarnings = FALSE)
    }
    ...
    p1 <- ggplot() +  
        ...
        annotate("rect", xmin = 0.5, xmax = 0.8, ymin = 200, ymax = 450,
                 alpha = .2, color="black", fill="white")+
        annotate("text", x=0.7, y=380, label=expression(aCO[2]), size=6)+
        annotate("text", x=0.58, y=380, label=expression(eCO[2]), size=6)+
        annotate("segment", x = 0.7, xend = 0.7, y = 210, yend = 310,
                 colour = "grey", size=1)+
        annotate("segment", x = 0.58, xend = 0.58, y = 210, yend = 310,
                 colour = "red", size=1)+
        annotate("segment", x = 0.66, xend = 0.74, y = 210, yend = 210,
                 colour = "grey", size=1)+
        annotate("segment", x = 0.66, xend = 0.74, y = 310, yend = 310,
                 colour = "grey", size=1)+
        annotate("segment", x = 0.54, xend = 0.62, y = 210, yend = 210,
                 colour = "red", size=1)+
        annotate("segment", x = 0.54, xend = 0.62, y = 310, yend = 310,
                 colour = "red", size=1)+
        geom_point(aes(x=0.7, y=260), color="black")+
        geom_point(aes(x=0.58, y=260), color="brown")
    ...
    pdf("output/overall_nutrient_uptake/summary_nutrient_uptake_overall_plot.pdf", width=16, height=5)
    grid.labs <- c("(a)", "(b)", "(c)", "(d)")
