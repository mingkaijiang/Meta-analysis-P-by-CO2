metafor_statistics_biomass_100_mixed_effect_model <- function(reDF, intDF) {
    ...
    mod1 <- gam(log_co2_aP ~ Trt_eCO2 + Trt_aCO2 + Trt_eP_by_aP, data=tDF, 
                weights = variance_co2_aP/mean(variance_co2_aP))
    pred1 <- predict(mod1, list(Trt_eCO2 = eco2.conc, Trt_aCO2 = aco2.conc, Trt_eP_by_aP = eP_aP), 
                     interval = "confidence", se.fit = TRUE)
    
    mod2 <- gam(log_co2_eP ~ Trt_eCO2 + Trt_aCO2 + Trt_eP_by_aP, data=tDF, 
                weights = variance_co2_eP/mean(variance_co2_eP))
    pred2 <- predict(mod2, list(Trt_eCO2 = eco2.conc, Trt_aCO2 = aco2.conc, Trt_eP_by_aP = eP_aP), 
                     interval = "confidence", se.fit = TRUE)
