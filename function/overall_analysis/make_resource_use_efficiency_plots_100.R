make_resource_use_efficiency_plots_100 <- function(inDF) {
    
    if(!dir.exists("output/overall_resource_use_efficiency_100")) {
        dir.create("output/overall_resource_use_efficiency_100", showWarnings = FALSE)
    }
    ...
    pDF1 <- data.frame(c(0.8, 1.2, 1.8, 2.2, 2.8, 3.2), NA, NA, NA, NA, NA)
    colnames(pDF1) <- c("brk", "variable", "CO2_trt", "value", "pos", "neg")
    pDF1$variable <- rep(c("NUE", "PUE", 
                           "WUE"),each=2)
    pDF1$CO2_trt <- rep(c("aC", "eC"),3)
    pDF2 <- pDF1
    ...
    ### create a dataframe to contain long format data for each plot (3 dataframes)
    pDF3 <- data.frame(c(1:3), NA, NA, NA, NA)
    colnames(pDF3) <- c("brk", "variable", "value", "pos", "neg")
    pDF3$variable <- c("NUE", "PUE", 
                       "WUE")
    ...
    p1 <- ggplot() +
        ...
        geom_errorbar(data=pDF1, 
                      ...
                      width=0.2, size=1) +
        ylab("P reduction RR") +
        scale_x_continuous(name = "Resource use efficiency",
                           breaks=c(1,2,3),
                           labels=c("NUE", "PUE", 
                                    "WUE"))+
        theme_linedraw() +
    ...
    pdf("output/overall_resource_use_efficiency_100/summary_resource_use_efficiency_overall_plot.pdf", width=6, height=12)
